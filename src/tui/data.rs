...
// Encrypt data
fn encrypt_data(data: &[u8], key: &[u8; 32]) -> Result<(Vec<u8>, Vec<u8>), String> {
...
pub fn load_passwords(file_path: &str, password: &str) -> Result<(Vec<PasswordEntry>, [u8; 32], Vec<u8>), String> {
    let mut file = File::open(file_path).map_err(|e| format!("Failed to open file: {}", e))?;
    let mut contents = String::new();
    file.read_to_string(&mut contents)
        .map_err(|e| format!("Failed to read file: {}", e))?;
    log::debug!("File contents: {}", contents);
    let encrypted: EncryptedFile = serde_json::from_str(&contents)
    ...
    let encrypted_data = STANDARD.decode(&encrypted.encrypted_data)
...
pub fn create_password_file(file_path: &str, password: &str) -> Result<(Vec<PasswordEntry>, [u8; 32], Vec<u8>), String> {
    let mut salt = [0u8; 16];
