        ...
        terminal.draw(|f| {
            ...
            f.render_widget(
                ratatui::widgets::Paragraph
                    ::new(format!("Enter master password: {}", "*".repeat(password.len())))
                    .block(
        ...
        let result = (|| -> io::Result<(Vec<PasswordEntry>, [u8; 32], Vec<u8>)> {
            if std::path::Path::new(PASSWORD_FILE_PATH).exists() {
                ...
                if first_char[0] == b'[' {
                    ...
                    let key: [u8; 32] = derive_key(&password, &salt).map_err(|e|
                        io::Error::new(io::ErrorKind::Other, e)
                    )?;

                    save_passwords(PASSWORD_FILE_PATH, &passwords, &key, &salt).map_err(|e|
                        io::Error::new(io::ErrorKind::Other, e)
                    )?;
                        ...
                        match load_passwords(PASSWORD_FILE_PATH, &password) {
                            ...
                            Err(e) => {
                                terminal.draw(|f| {
                                    ...
                                    f.render_widget(
                                        ratatui::widgets::Paragraph
                                            ::new(format!("Error: Invalid password - {}", e))
                                            .block(
                                                ratatui::widgets::Block
                                                    ::default()
                                                    .borders(ratatui::widgets::Borders::ALL)
                ...
                terminal.draw(|f| {
                    ...
                    f.render_widget(
                        ratatui::widgets::Paragraph
                            ::new("No password file found. Setting up new file...")
                            .block(
                                ratatui::widgets::Block
                                    ::default()
                                    .borders(ratatui::widgets::Borders::ALL)
