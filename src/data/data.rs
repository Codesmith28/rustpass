...
fn decrypt_data(ciphertext: &[u8], key: &[u8; 32], nonce: &[u8]) -> DecryptedDataResult {
    let cipher = Aes256Gcm::new(key.into());
    cipher.decrypt(nonce.into(), ciphertext).map_err(|e| format!("Decryption failed: {}", e))
}
...
pub fn load_passwords(file_path: &str, password: &str) -> PasswordDataResult {
    ...
    let encrypted: EncryptedFile = serde_json
        ::from_str(&contents)
        .map_err(|e| format!("Invalid file format: {}", e))?;
    //log::debug!("Encrypted salt: {}", encrypted.salt);

    let salt = STANDARD.decode(&encrypted.salt).map_err(|e| format!("Invalid salt: {}", e))?;
    //log::debug!("Decoded salt: {:?}", salt);

    let nonce = STANDARD.decode(&encrypted.nonce).map_err(|e| format!("Invalid nonce: {}", e))?;
    ...
    let passwords: Vec<PasswordEntry> = serde_json
        ::from_slice(&decrypted)
...
pub fn save_passwords(
    ...
    file
        .write_all(serde_json::to_string(&encrypted_file).unwrap().as_bytes())
...
pub fn load_passwords_with_key(
    ...
    let mut file = File::open(file_path).map_err(|e|
        io::Error::new(io::ErrorKind::NotFound, format!("Failed to open file: {}", e))
    )?;
    let mut contents = String::new();
    ...
    file
        ...
        .map_err(|e|
            io::Error::new(io::ErrorKind::InvalidData, format!("Failed to read file: {}", e))
        )?;

    let encrypted: EncryptedFile = serde_json
        ::from_str(&contents)
        .map_err(|e|
            io::Error::new(io::ErrorKind::InvalidData, format!("Invalid file format: {}", e))
        )?;

    let nonce = STANDARD.decode(&encrypted.nonce).map_err(|e|
        io::Error::new(io::ErrorKind::InvalidData, format!("Invalid nonce: {}", e))
    )?;

    let encrypted_data = STANDARD.decode(&encrypted.encrypted_data).map_err(|e|
        io::Error::new(io::ErrorKind::InvalidData, format!("Invalid encrypted data: {}", e))
    )?;
    ...
    let passwords: Vec<PasswordEntry> = serde_json
        ::from_slice(&decrypted)
        .map_err(|e|
            io::Error::new(io::ErrorKind::InvalidData, format!("Failed to parse passwords: {}", e))
        )?;
